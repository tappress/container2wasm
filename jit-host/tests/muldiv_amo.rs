...
fn run_muldiv(kind: u8, a: u64, b: u64) -> u64 {
    ...
    let (bytes, used) = codegen::build_block(&ir, 0x1000, 0x1004, 1, None, None, false);
...
fn muldiv_mid_block_threading() {
    ...
    let (bytes, used) = codegen::build_block(&ir, 0x2000, 0x200c, 3, None, None, false);
...
fn run_amo(ir: &[u8], regs: &[(usize, u64)], fail: bool, expect_used: &[usize]) -> (i64, AmoCalls) {
    let (bytes, used) = codegen::build_block(ir, 0x3000, 0x3000 + 4 * 4, 4, None, None, false);
